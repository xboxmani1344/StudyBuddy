...
REM "OPENROUTER_API_KEY is not set", which tells the user nothing about the cause.
node -e "require('dotenv').config();process.exit(process.env.OPENROUTER_API_KEY?0:1)" 2>nul
if not errorlevel 1 goto :havekey

if exist ".env" goto :badkeyfile
echo.
echo [2/2] One-time setup: your free OpenRouter API key.
goto :askkey
...
>> ".env" echo OPENROUTER_API_KEY=%GEMKEY%
